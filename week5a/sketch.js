...
let ballX;//var to remember ball position
let ballSpeed = 0.5;
let ballDiameter = 30;
...
function draw() {
  ...
  ballX = ballX + ballSpeed;
  //In the above example, we are incrementing ballX by a quantity of ballSpeed.
  //These variables are global and set at the top
  circle(ballX, 200, 30);//circle uses ballX for x pos
  
}

function mousePressed() {
  //the mousePressed function runs enclosed code ONCE whent the mouse is pressed down
  if (hovering == true) {
  r = random(255);
  g = random(255);
  b = random(255);
